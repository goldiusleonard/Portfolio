...
def test_get_visual_chart_data_prediction() -> None:
    """Test the get_visual_chart_data function for prediction state."""
    categories = ["Category1", "Category2"]
    period = 1
    chart_type = "engagement"
    state = "prediction"
    ...
    with patch(
        "app.api.endpoints.functions.category_function.__get_prediction_data",
        return_value=mock_response,
    ):
        result = category_function.get_visual_chart_data(
            categories,
