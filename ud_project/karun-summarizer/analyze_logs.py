# Log Analysis Tool for Karun API
...
def find_log_files(log_dir: str = "logs") -> List[str]:
    base_pattern = os.path.join(log_dir, "karun_api_*.log*")
    log_files = [
        f
...
# Extract processing times from API log files (matching 'Processing time: <ms>ms')
def extract_processing_times(log_files: List[str]) -> List[float]:
    times = []
    pattern = r"Processing time: (\d+)ms"
    ...
    for log_file in log_files:
        try:
            with open(log_file, encoding="utf-8") as f:
                ...
                times.extend(
                    [
                        float(match.group(1)) / 1000  # Convert ms to seconds
                        for line in f
                        if (match := re.search(pattern, line))
                    ],
                )
        except Exception as e:
            print(f"Error processing {log_file}: {e!s}")
...
def plot_histogram(times: List[float], output_dir: str = "output"):
    ...
    bins = range(max_time + 2)

    # Create histogram
    plt.hist(times, bins=bins, edgecolor="black", align="left")
    plt.title("Distribution of Karun Agent Processing Times")
    ...
    output_path = os.path.join(output_dir, f"karun_processing_times_{timestamp}.png")
...
def main():
    ...
    if not log_files:
        print("No log files found matching pattern 'karun_api_*.log*'")
        return
